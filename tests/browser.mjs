...
const fPng = await dlFmt('png');  ok('⬇ Download → Image sheet saves .png', !!fPng && /\.png$/.test(fPng));
const fSvg = await dlFmt('svg');  ok('⬇ Download → Vector board saves .svg', !!fSvg && /\.svg$/.test(fSvg));
const popup = page.waitForEvent('popup', { timeout: 8000 });
await page.click('#dl-btn'); await page.click('#dl-menu [data-fmt="pdf"]');
const pp = await popup.catch(()=>null);
ok('⬇ Download → Print/PDF opens a print page with the play', !!pp && /Triibholz/.test(await pp.title().catch(()=>'')));
...
ok('the test log downloads as a real CSV file', !!devDlFile && /\.csv$/.test(devDlFile.suggestedFilename()));
